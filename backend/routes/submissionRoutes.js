...
// Debug endpoint to check mentor submissions
router.get("/debug/mentor", protect, async (req, res) => {
  try {
    ...
    console.log('Debug: Mentor submissions request for user:', req.user._id);
    
    // Find all tests created by this mentor
    const mentorTests = await Test.find({ createdBy: req.user._id }).select('_id name');
    console.log('Debug: Found mentor tests:', mentorTests.length);
    
    const testIds = mentorTests.map(test => test._id);
    console.log('Debug: Test IDs:', testIds);
...
router.get("/mentor", protect, async (req, res) => {
  try {
    ...
    console.log('Fetching mentor submissions for user:', req.user._id);
    
    // Find all tests created by this mentor
    const mentorTests = await Test.find({ createdBy: req.user._id }).select('_id');
    const testIds = mentorTests.map(test => test._id);
    
    console.log('Found', testIds.length, 'tests for mentor');
    ...
    console.log('Found', submissions.length, 'submissions for mentor tests');
...
// Get single submission by ID (This must be LAST to avoid conflicting with other routes)
